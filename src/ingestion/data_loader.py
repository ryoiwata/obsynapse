...
class ChunkBuilder:
    """
    Structured chunk representation for merge-before-render pipeline.
    """
    # Scope information
    chunk_scope: str
    scope_key: str
    ...
    # Metadata
    chunking_decisions: List[str] = field(default_factory=list)
    chunk_index: int = 0
    
    # Derived (computed during finalization)
    chunk_id: str = ""
    content_hash: str = ""
    scope_hash: str = ""
    block_types: List[str] = field(default_factory=list)
    chunk_role: str = ""
    has_optional_blocks: bool = False
    list_item_count: Optional[int] = None
    list_group_index: Optional[int] = None
    breadcrumb: str = ""
    hierarchy: Dict[str, Any] = field(default_factory=dict)
    
    # Rendered text (computed once at finalization)
    embed_text: str = ""
    study_text: str = ""
    
    def compute_size(self) -> int:
        """Estimate chunk size from blocks."""
        total = 0
        for block in self.blocks:
            block_text = _format_block_text(block)
            total += len(block_text) + 2  # +2 for separators
        return total
    
    def can_merge_with(self, other: 'ChunkBuilder') -> bool:
        """Check if this chunk can be merged with another (scope-safe)."""
        return (
            self.chunk_scope == other.chunk_scope and
            self.scope_key == other.scope_key and
            # For subheads, also require identical subhead_path
            (self.chunk_scope != "subhead" or self.subhead_path == other.subhead_path)
        )
    
    def merge_with(self, other: 'ChunkBuilder', decision: str) -> 'ChunkBuilder':
        """Merge another chunk into this one."""
        merged = ChunkBuilder(
            chunk_scope=self.chunk_scope,
            scope_key=self.scope_key,
            chapter_title=self.chapter_title,
            subsection_index=self.subsection_index,
            subsection_title=self.subsection_title,
            subhead_path=self.subhead_path,
            source_path=self.source_path,
            blocks=self.blocks + other.blocks,
            block_refs=self.block_refs + other.block_refs,
            chunking_decisions=self.chunking_decisions + [decision] + other.chunking_decisions,
...
def chunk_from_structure(
    ...
    New pipeline: merge-before-render with structured chunks.
    Prevents cross-scope duplication, double headers, and ensures
    deterministic/idempotent merges with reliable metadata.
    ...
    # A. Build initial structured chunks
    structured_chunks = _build_initial_chunks_from_structure(doc_structure)
    
    # B. Bridge merge pass (structured)
    structured_chunks = _apply_bridge_merges(structured_chunks)
    
    # C. Minimum chunk quality pass (structured) - BEFORE hard flush
    structured_chunks = _apply_min_quality_merge(structured_chunks)
    
    # D. Max-size splitting (structured, safe boundaries)
    structured_chunks = _apply_max_size_splitting(structured_chunks)
    
    # E. Finalize metadata + render once
    chunks, chunk_metadata = _finalize_chunk_meta_and_render(
        structured_chunks
    )
...
# ============================================================================
# NEW STRUCTURED CHUNKING PIPELINE
# ============================================================================

def _build_initial_chunks_from_structure(
    doc_structure: DocumentStructure
) -> List[ChunkBuilder]:
    """
    Build initial structured chunks from document structure.
    
    Implements rule: if subsection has subheads, only chunk intro blocks
    (before first subhead) at subsection level. Never include subhead blocks
    in subsection chunks.
    """
    if doc_structure.chapter is None:
        return []
    ...
    if chapter.blocks_before_subsections:
        ...
        structured_chunks.extend(preface_chunks)
    ...
    for subsection in chapter.subsections:
        if subsection.subheads:
            ...
            # CRITICAL FIX: Only chunk intro blocks (before first subhead)
            # at subsection level. These are the blocks in subsection.blocks
            # that come BEFORE any subhead blocks.
            if subsection.blocks:
                _mark_optional_blocks(subsection.blocks)
                
                # Build subsection-level chunks from intro blocks only
                intro_chunks = _build_structured_chunks_from_blocks(
                ...
                structured_chunks.extend(intro_chunks)
                chunk_index += len(intro_chunks)
            
            # Chunk each subhead separately (never include in subsection)
            for subhead in subsection.subheads:
                subhead_chunks = _build_structured_chunks_from_blocks(
                ...
                structured_chunks.extend(subhead_chunks)
                chunk_index += len(subhead_chunks)
        else:
            # No subheads, chunk subsection blocks directly
            _mark_optional_blocks(subsection.blocks)
            subsection_chunks = _build_structured_chunks_from_blocks(
                blocks=subsection.blocks,
                chapter_title=chapter.title,
                subsection_index=subsection.order_index,
                subsection_title=subsection.title,
            ...
            structured_chunks.extend(subsection_chunks)
...
def _build_structured_chunks_from_blocks(
    blocks: List[Block],
    chapter_title: str,
    subsection_index: Optional[int],
    subsection_title: Optional[str],
    ...
    """
    Build structured chunks from blocks within a scope.
    
    Creates ChunkBuilder objects with blocks, not rendered text.
    Hard boundaries create new chunks, but merges happen later.
    """
    if not blocks:
        return []
    ...
    scope_key = _get_scope_key(chunk_scope, subsection_index, subhead_path)
    ...
    while i < len(blocks):
        ...
        if block.type in ("callout", "code", "quote", "media"):
            ...
            if (current_chunk_blocks and
                    len(current_chunk_blocks) == 1 and
                    current_chunk_blocks[0].type == "paragraph"):
                lead_in_text = _format_block_text(current_chunk_blocks[0])
                if _is_lead_in_paragraph(lead_in_text, block):
                    should_attach_lead_in = True
            
            if should_attach_lead_in:
                # Merge lead-in into this chunk
                ...
                if current_chunk_blocks:
                    chunk = ChunkBuilder(
                        ...
                        block_refs=current_chunk_block_refs.copy(),
                        chunking_decisions=current_chunk_decisions.copy(),
                        chunk_index=chunk_index
                    )
                ...
                if block.type != "media" or block.text:  # Skip empty media
        ...
        # Lists
            ...
            if should_attach_lead_in:
                ...
                for group_idx, item_group in enumerate(item_groups):
                    # Create a block-like structure for the group
                    list_block = Block(
                        type="list",
                        text="\n".join(f"- {item}" for item in item_group),
                        meta={
                            **block.meta,
                            "items": item_group,
                    ...
                    chunk = ChunkBuilder(
                        ...
                        chunking_decisions=["list_split"] if len(item_groups) > 1 else [],
                        chunk_index=chunk_index
                    )
                    structured_chunks.append(chunk)
                    chunk_index += 1
        ...
        # Paragraphs
            ...
            chunk_size = sum(
                len(_format_block_text(b)) + 2
                for b in current_chunk_blocks
            )
            if chunk_size > CHUNK_TARGET_MAX:
                # Flush current chunk
                chunk = ChunkBuilder(
                    chunk_scope=chunk_scope,
                    scope_key=scope_key,
...
def _apply_bridge_merges(chunks: List[ChunkBuilder]) -> List[ChunkBuilder]:
    """
    Apply bridge merges: lead-in → list, quote bridge, label anchoring.
    Operates on structured chunks.
    """
    if len(chunks) <= 1:
        return chunks
    ...
    merged = []
    i = 0
    touched = set()  # Track merged chunks
    ...
    while i < len(chunks):
        ...
        # Check for quote/callout + short paragraph bridge
        if (i + 1 < len(chunks) and
                current.blocks and
                current.blocks[-1].type in ("quote", "callout")):
            next_chunk = chunks[i + 1]
            if (next_chunk.can_merge_with(current) and
                    next_chunk.blocks and
                    next_chunk.blocks[0].type == "paragraph"):
                para_text = _format_block_text(next_chunk.blocks[0])
                if _is_short_paragraph(para_text):
                    # Merge
                    merged_chunk = current.merge_with(
                        next_chunk,
                        f"{current.blocks[-1].type}_bridge_merge_forward"
                    )
        ...
        # Check for label paragraph before list/quote/callout/code
        if (i + 1 < len(chunks) and
                current.blocks and
                len(current.blocks) == 1 and
                current.blocks[0].type == "paragraph"):
            para_text = _format_block_text(current.blocks[0])
            next_chunk = chunks[i + 1]
            if (next_chunk.can_merge_with(current) and
                    next_chunk.blocks and
                    next_chunk.blocks[0].type in ("list", "quote", "callout", "code")):
                if _is_lead_in_paragraph(para_text, next_chunk.blocks[0]):
                    # Merge
                    merged_chunk = current.merge_with(
                        next_chunk,
                        "label_anchor_merge_forward"
                    )
                    merged.append(merged_chunk)
                    touched.add(i)
                    touched.add(i + 1)
...
def _apply_min_quality_merge(chunks: List[ChunkBuilder]) -> List[ChunkBuilder]:
    """
    Merge tiny chunks below MIN_CHUNK_CHARS (except preface).
    Runs BEFORE hard flush to avoid fractures.
    """
    if len(chunks) <= 1:
        return chunks
    ...
    while i < len(chunks):
        ...
        current = chunks[i]
        chunk_size = current.compute_size()
        
        is_tiny = (
            chunk_size < MIN_CHUNK_CHARS and
            current.chunk_scope != "preface"
        )
        ...
        # Try to merge backward
        if merged and merged[-1].can_merge_with(current):
            prev = merged.pop()
            combined_size = prev.compute_size() + current.compute_size()
            if combined_size < CHUNK_HARD_MAX_CHARS:
                merged_chunk = prev.merge_with(current, "min_quality_merge_backward")
                merged.append(merged_chunk)
                touched.add(i)
                i += 1
                continue
        ...
        # Try to merge forward
        if (i + 1 < len(chunks) and
                chunks[i + 1].can_merge_with(current)):
            next_chunk = chunks[i + 1]
            # Check if next starts with hard boundary or list
            if next_chunk.blocks:
                first_type = next_chunk.blocks[0].type
                if first_type in ("list", "quote", "callout", "code"):
                    combined_size = current.compute_size() + next_chunk.compute_size()
                    if combined_size < CHUNK_HARD_MAX_CHARS:
                        merged_chunk = current.merge_with(
                            next_chunk,
                            "min_quality_merge_forward"
...
def _apply_max_size_splitting(chunks: List[ChunkBuilder]) -> List[ChunkBuilder]:
    """
    Split chunks that exceed CHUNK_HARD_MAX_CHARS at safe boundaries.
    Never splits inside code/callout.
    """
    result = []
    
    for chunk in chunks:
        chunk_size = chunk.compute_size()
        
        if chunk_size <= CHUNK_HARD_MAX_CHARS:
            result.append(chunk)
            continue
        
        # Split at safe boundaries
        # Strategy: split at paragraph boundaries, list groups, etc.
        split_points = []
        current_size = 0
        
        for i, block in enumerate(chunk.blocks):
            block_size = len(_format_block_text(block)) + 2
            
            # Never split inside code/callout
            if block.type in ("code", "callout"):
                current_size += block_size
                continue
            
            # Safe split points: after paragraph, list, quote
            if block.type in ("paragraph", "list", "quote"):
                if current_size + block_size > CHUNK_HARD_MAX_CHARS and split_points:
                    # Can split after previous block
        ...
        if not split_points:
            # Can't split safely, keep as-is (will be handled by embedding)
            result.append(chunk)
            continue
        
        # Create split chunks
        split_points = [0] + split_points + [len(chunk.blocks)]
        for j in range(len(split_points) - 1):
            start = split_points[j]
            end = split_points[j + 1]
            split_chunk = ChunkBuilder(
                chunk_scope=chunk.chunk_scope,
                scope_key=chunk.scope_key,
                chapter_title=chunk.chapter_title,
                subsection_index=chunk.subsection_index,
                subsection_title=chunk.subsection_title,
                subhead_path=chunk.subhead_path,
                source_path=chunk.source_path,
                blocks=chunk.blocks[start:end],
                block_refs=chunk.block_refs[start:end],
                chunking_decisions=chunk.chunking_decisions + ["max_size_split"],
...
def _finalize_chunk_meta_and_render(
    chunks: List[ChunkBuilder]
) -> Tuple[List[str], List[Dict[str, Any]]]:
    """
    Finalize metadata for all chunks and render once.
    Ensures metadata is reliable and headers are not duplicated.
    """
    chunk_texts = []
    chunk_metadata = []
    ...
    for chunk in chunks:
        # Finalize metadata
        _finalize_chunk_meta(chunk)
        
        # Render once
        embed_text, study_text = _render_chunk(chunk)
        chunk.embed_text = embed_text
        chunk.study_text = study_text
        
        # Validate (log warnings, don't crash)
        _validate_chunk(chunk)
        
        # Convert to output format
        chunk_texts.append(embed_text)
        chunk_metadata.append(_chunk_builder_to_dict(chunk))
    
    return chunk_texts, chunk_metadata


def _finalize_chunk_meta(chunk: ChunkBuilder) -> None:
    """
    Recompute and finalize chunk metadata from actual blocks.
    Ensures block_refs, block_types, and derived fields are correct.
    """
    # Ordered-dedupe block_refs
    chunk.block_refs = list(dict.fromkeys(chunk.block_refs))
    
    # Recompute block_types from actual blocks
    chunk.block_types = list(dict.fromkeys([b.type for b in chunk.blocks]))
    
    # Recompute has_optional_blocks
    chunk.has_optional_blocks = any(
        b.meta.get("is_optional", False) for b in chunk.blocks
    )
    
    # Recompute chunk_role
    chunk.chunk_role = _compute_chunk_role(
        chunk.blocks,
        chunk.chunk_scope,
        chunk.has_optional_blocks
    )
    
    # Recompute list metadata
    list_blocks = [b for b in chunk.blocks if b.type == "list"]
    if list_blocks:
        total_items = sum(
            len(b.meta.get("items", [])) for b in list_blocks
        )
        chunk.list_item_count = total_items
        if len(list_blocks) > 1:
            chunk.list_group_index = 0  # Simplified
    
    # Recompute hashes and IDs
    chunk.scope_hash = _compute_scope_hash(
        chunk.source_path,
        chunk.chunk_scope,
        chunk.subsection_index,
        chunk.subhead_path
    )
    ...
    # Recompute breadcrumb and hierarchy
    subhead_title = None
    if chunk.subhead_path and len(chunk.subhead_path) > 2:
        subhead_title = chunk.subhead_path[-1]
    
    chunk.breadcrumb = _format_breadcrumb(
        chunk.chapter_title,
        chunk.subsection_index,
    ...
    # Ordered-dedupe chunking_decisions
    chunk.chunking_decisions = list(dict.fromkeys(chunk.chunking_decisions))
    
    # Compute content hash (will be done after rendering)
    # This is a placeholder - actual hash computed in _render_chunk


def _render_chunk(chunk: ChunkBuilder) -> Tuple[str, str]:
    """
    Render chunk to embed_text and study_text.
    Headers are added ONCE here, never in block rendering.
    """
    # Get subhead title
    subhead_title = None
    if chunk.subhead_path and len(chunk.subhead_path) > 2:
    ...
    # Render blocks to markdown (NO headers in blocks)
    body_parts = []
    ...
    return embed_text, study_text


def _validate_chunk(chunk: ChunkBuilder) -> None:
    """
    Validate chunk invariants. Log warnings, don't crash.
    """
    # Check block_refs vs block_types
    if len(chunk.block_refs) > 0 and len(chunk.block_types) == 0:
        logger.warning(
            f"Chunk {chunk.chunk_index} has block_refs but no block_types"
        )
    
    # Check for duplicate headers in rendered text
    if chunk.embed_text:
        header_count = chunk.embed_text.count("Chapter:")
        if header_count > 1:
            logger.warning(
                f"Chunk {chunk.chunk_index} has duplicate Chapter: headers"
            )


def _chunk_builder_to_dict(chunk: ChunkBuilder) -> Dict[str, Any]:
    """Convert ChunkBuilder to metadata dict."""
    return {
        "chunk_index": chunk.chunk_index,
        ...
        "chapter_title": chunk.chapter_title,
        "subsection_index": (
            str(chunk.subsection_index)
            if chunk.subsection_index is not None
            else None
        ),
        "subsection_title": chunk.subsection_title,
        "subhead_path": (
            " > ".join(chunk.subhead_path)
            if chunk.subhead_path
            else None
        ),
        "chunk_scope": chunk.chunk_scope,
        "scope_key": chunk.scope_key,
        ...
        "chunking_decisions": chunk.chunking_decisions,
        "has_optional_blocks": chunk.has_optional_blocks,
        "chunk_role": chunk.chunk_role,
        "breadcrumb": chunk.breadcrumb,
        "hierarchy": chunk.hierarchy,
        "embed_text": chunk.embed_text,
        "study_text": chunk.study_text,
        "list_item_count": chunk.list_item_count,
        "list_group_index": chunk.list_group_index
    }


# ============================================================================
# OLD FUNCTIONS (kept for reference, may be removed later)
# ============================================================================
